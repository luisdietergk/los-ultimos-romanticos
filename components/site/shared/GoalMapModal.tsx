...
export function GoalMapModal({
  ...
  // (open stays true the whole time) precisely so Radix never starts its own
  // close transition from translateY(0) mid-drag, which would jump the sheet
  // back up first. Reused by both the drag gesture and Radix's own dismiss
  // triggers (outside tap, Escape) via onOpenChange below.
